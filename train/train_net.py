...
def train_net(net, train_path, num_classes, batch_size,
    ...
    if resume > 0:
        ...
        try:
            ...
            print('[*] Load Pretrained From CheckPoint : {}'.format(pretrained))

        except:

            print('[*] Load Pretrained From NDArray : {}'.format(pretrained))

            nds = mx.nd.load(pretrained)
            
            args = dict()

            auxs = dict()

            vargs = net.list_arguments()
            ...
            for n in nds.keys():
                ...
                if n.endswith('weight') or n.endswith('bias'):
                    ...
                    print('{} : {}'.format(ns, nds[n].shape))

                elif n.endswith('gamma') or n.endswith('mean') or n.endswith('var'):
