...
def senet_block(feat, name, num_filter, num_output, stride, num_group, match=False):
    ...
    down = mx.symbol.Convolution(name=name + '_1x1_down', data=global_pool, num_filter= int(num_output / 16),
                                 kernel=(1, 1),
...
def get_symbol(num_classes, model, num_group=32, conv_workspace=256, dtype='float32', **kwargs):
    print(kwargs)
    if model == 50:
        stages = [3, 4, 6, 3]
    elif model == 101:
    ...
    ########## seres1-seres16 #########
    stage1 = stage(start_num=1, num_block=stages[0], feat=pool1, num_filter=int(num_group*2), num_output=num_output[0],
