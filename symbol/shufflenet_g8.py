...
def shuffleUnit(residual, in_channels, out_channels, combine_type, groups=3, grouped_conv=True, stage = '', repeat = ''):
    ...
    data = mx.sym.Convolution(data=residual, num_filter = bottleneck_channels, 
                      ...
                      name = 'stage{}/block{}/conv{}'.format(stage, repeat, 1))

    #print(data.name)

    data = mx.sym.BatchNorm(data = data, name = 'stage{}/block{}/conv{}/bn'.format(stage, repeat, 1))
    ...
    data = mx.sym.Convolution(data=data, num_filter = bottleneck_channels, kernel = (3, 3), 
                      ...
                      name = 'stage{}/block{}/dconv'.format(stage, repeat))
    
    data = mx.sym.BatchNorm(data = data, name = 'stage{}/block{}/dconv/bn'.format(stage, repeat))

    data = mx.sym.Convolution(data=data, num_filter=out_channels, 
    	               kernel=(1, 1), stride=(1, 1), num_group=groups,
                      name = 'stage{}/block{}/conv{}'.format(stage, repeat, 2))

    data = mx.sym.BatchNorm(data = data, name = 'stage{}/block{}/conv{}/bn'.format(stage, repeat, 2))
...
def make_stage(data, stage, groups = 8):
    stage_repeats = [3, 7, 3]
...
def get_symbol(num_classes, **kwargs):
    ...
    #out = mx.sym.SoftmaxOutput(data = data, name = 'softmax', use_ignore = True)
