...
if __name__ == '__main__':
    args = parse_args()
    det, nms = get_symbol_split(args.network, args.data_shape,
    ...
    save_prefix_nms = save_prefix + '-nms'

    mx.model.save_checkpoint(save_prefix_det , args.epoch, det, arg_params, aux_params)
    
    nms.save('{}-symbol.json'.format(save_prefix_nms))

    print("Saved model: {}-{:04d}.params".format(save_prefix_det, args.epoch))
    
    print("Saved symbol: {}-symbol.json".format(save_prefix_det))
    
    print("Saved symbol: {}-symbol.json".format(save_prefix_nms))
