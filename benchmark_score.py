...
def get_data_shapes(batch_size, shape):
    image_shape = (3, shape, shape)
    return [('data', (batch_size,)+image_shape)]

def get_data(batch_size, args_shape):
    data_shapes = get_data_shapes(batch_size, args_shape)
...
if __name__ == '__main__':
    ...
    b = args.batch_size
    supported_image_shapes = [300, 512, 224]
    supported_networks = ['vgg16_reduced', 'inceptionv3', 'resnet50', 'mobilenet_v2', 'shufflenet']
    ...
    for bs in batch_sizes:
        batch = get_data(bs, args.shape)
        mod = mx.mod.Module(net, label_names=None, context=mx.cpu())
        mod.bind(for_training = False,
                 inputs_need_grad = False,
                 data_shapes = get_data_shapes(bs, args.shape))
